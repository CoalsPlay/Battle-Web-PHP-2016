				...
				<div class="panel panel-default">
				  ...
				  <div class="panel-body">
					<div class="list-group">
					  ...
					  <a href="<?php echo $web['url']; ?>/mensajes_enviados" class="list-group-item active">
					  ...
					  <tbody>
					  	<?php
					  		$sql = mysqli_query($conexion, "SELECT * FROM mensajes_privados
					  										WHERE id_autor_mp = '$dato_u[id]'
					  										ORDER BY id_mp DESC LIMIT $inicio, $limite");
					  		if(mysqli_num_rows($sql) == 0){

					  			echo '<center><h3>No has enviado ningún mensaje.</h3></center><br/>';
					  		}else{
					    ...
					    <tr>
					      ...
					      <td><a href="<?php echo $web['url']; ?>/perfil/<?php echo $row['nombre_receptor']; ?>"><?php echo $row['nombre_receptor']; ?></a></td>
						...
						<?php
							...
							if ($total_pag > 1){
							    for ($i=1;$i<=$total_pag;$i++){
							       if ($pag == $i)
							          echo '<li class="active"><a href="'.$web['url'].'/mensajes_enviados/'.$i.'">'.$pag.'</a></li>';
							       else
							       		echo '<li><a href="'.$web['url'].'/mensajes_enviados/'.$i.'">'.$i.'</a></li>';
							    }
